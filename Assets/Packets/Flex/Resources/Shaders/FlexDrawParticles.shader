...
void geom_flex(point v2g_flex IN[1], uint id : SV_PrimitiveId, inout TriangleStream<g2f_flex> OUT)
{
    ...
    //unity_CameraToWorld = UNITY_MATRIX_I_V
...
f2o_flex frag_flex(g2f_flex IN)
{
    ...
    //方向为零点（摄像机）出发，指向p点，长度为1/_Radius
    //IN.vpos在摄像机空间
    //除以_Radius后可以把d于p看做同一空间（摄像机空间经过一个相似比为_Radius的相似变换）下的向量
    float3 d = normalize(IN.vpos) / _Radius;
    //解关于t的方程|dt+p|_2=1
    //方程的含义：向量d从球表面经历时间t后到达前述平面（且由上已知交点指向圆心的向量为p）
    //在该坐标系内球的半径为1
    ...
    //计算深度，d为视空间中与xoy平行且球心所在平面上一点的坐标，t为从球表面进入到该平面所花的时间（视空间的一个相似空间）
    float z = -d.z * t;
    float depth = IN.vpos.z - z * _Radius;

    //这个是用Reversed-Z进行深度缓冲，防止Z-Fighting
    //pz = (1/near)-(1/far)
    //pw = 1/far
...
f2o_flex frag_flex(g2f_flex IN)
{
    ...
    float pz = _ZBufferParams.z, pw = _ZBufferParams.w;
    o.d = (1 / depth - pw) / pz;
    //o.c = float4((z.xxx * 0.6 + 0.4) * _Color.xyz, 1) * (IN.mass > 0 ? 1.0 : 0.5);
    ...
    Pass {
        ...
        Cull off
            ...
            void geom(point v2g p[1], uint id : SV_PrimitiveId, inout TriangleStream<g2f> triStream)
            {
                ...
                float3 up = cam[1], right = cam[0], forward = cam[0];

                float radius = _Radius;

                float halfS = radius * 1.5;
                ...
                for (int i = 0; i < 4; ++i)
                {
                    pIn.pos = mul(UNITY_MATRIX_VP, v[i]);
                    pIn.tex0 = tex0[i];
                    pIn.vpos = mul(UNITY_MATRIX_V, v[i]);
            ...
            f2o frag(g2f input)
            {
                ...
                float3 d = normalize(input.vpos) / radius;
                ...
                float3 vpos = input.vpos - float3(0, 0, z * radius);
                float3 vnrm = normalize(p - float3(0, 0, z));
