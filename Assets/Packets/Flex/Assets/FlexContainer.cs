    ...
    public class FlexContainer : ScriptableObject
    {
        ...
        void CreateContainer()
        {
            ...
            if (m_containerHandle)
            {
                ...
                //在样例1中删除多余的FlexScene
                while(GameObject.Find("FlexScene"))
                {
                    GameObject.DestroyImmediate(GameObject.Find("FlexScene"));
                }
        ...
        /// <summary>
        /// 这个方法会以委派的形式添加到FlexScene的fixedUpdate事件中，
        /// 后者会在FlexScene的每一帧FixedUpdate()中被调用（且只有此方法会被添加到其中）
        /// 此方法为实际的每帧FixedUpdate()入口
        /// </summary>
        void FixedUpdate()
        {
            UpdateParams();
        ...
        /// <summary>
        /// 这个方法会以委派的形式添加到FlexScene的update事件中，
        /// 后者会在FlexScene的每一帧Update()中被调用（且只有此方法会被添加到其中）
        /// 此方法为实际的每帧Update()入口
        /// </summary>
        void Update()
        {
